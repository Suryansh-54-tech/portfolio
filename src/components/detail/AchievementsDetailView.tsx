        ...
        <Card variant="glass" className="h-full flex flex-col group relative overflow-hidden">
          ...
          <div className="p-5 flex flex-col h-full">
            <div className="flex items-start justify-between gap-4 mb-3">
              <ProviderLogo provider={cert.provider} size={36} />
              <Badge
                variant="outline"
                size="sm"
                className={cn(
            ...
            <p className="text-xs text-muted mb-2">Issued by {cert.issuedBy}</p>
            ...
            {cert.description && (
              <p className="text-xs text-muted/80 leading-relaxed mb-3 flex-1">
                {cert.description}
              </p>
            )}
            ...
            <div className="flex items-center gap-2 pt-3 border-t border-border">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 glass rounded-xl text-xs font-medium text-primary hover:bg-primary/10 transition-all border border-primary/30 group"
              >
              ...
              {cert.pdfUrl && (
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 glass rounded-xl text-xs font-medium text-muted hover:text-foreground hover:bg-secondary/50 transition-all border border-border group"
                >
              ...
              {!cert.pdfUrl && (
                <div className="flex-1 flex items-center justify-center gap-2 px-3 py-2 glass rounded-xl text-xs font-medium text-muted/50 border border-border/50">
...
export function AchievementsDetailView() {
  return (
    <div className="max-w-7xl">
      ...
      <motion.div
        ...
        className="mt-10 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3"
      >
          ...
          return (
            ...
            >
              <Card variant="glass" className="p-4 h-full text-center hover:border-primary/30 transition-all group">
                <div
                  className="w-10 h-10 mx-auto mb-2 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"
                  style={{ background: `linear-gradient(135deg, ${config.color}20, ${config.color}40)` }}
                >
                  <ProviderLogo provider={key} size={20} />
                </div>
                <h4 className="font-semibold text-xs mb-1">{config.label}</h4>
